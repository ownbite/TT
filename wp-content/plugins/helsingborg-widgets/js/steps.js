jQuery(document).ready(function($) {
  ...
  $('.pagination li').on('click', function(e) {
    if (this.innerText.length <= 2 && this.innerText.length > 0) {
      e.preventDefault();

      var currentElem = $('.guide-list li.current');
      var newElem = $('.guide-list li').eq(parseInt(this.innerText) - 1);
  ...
  function setPager() {
    var newIndex = $('.guide-list li').index($('.guide-list li.current'));
